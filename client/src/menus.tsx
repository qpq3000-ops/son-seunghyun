...
export const MENUS: MenuDef[] = [
  ...
  m('sale', '영업', '판매입력/조회', 1),
  m('sale-status', '영업', '판매현황', 1),
  m('receipt', '영업', '수금입력', 1),
  m('receivable', '영업', '미수금현황', 1),
  m('message', '영업', '거래처 메시지', 4),

  m('po', '구매', '발주서입력/조회', 1),
  m('purchase', '구매', '구매입력/조회', 1),
  m('purchase-status', '구매', '구매현황', 1),
  m('payment', '구매', '지불입력', 1),
  m('payable', '구매', '미지급금현황', 1),
  m('bean-price', '구매', '생두 단가비교', 4),

  m('bom', '생산', 'BOM등록', 2),
  m('roast-sheet', '생산', '로스팅시트(작업지시)', 2),
  m('prod-in', '생산', '생산입고', 2),
  m('prod-status', '생산', '생산현황/수율분석', 2),
  m('mrp', '생산', '소요량계산', 2),

  m('stock-status', '재고', '재고현황', 1),
  m('stock-wh', '재고', '창고별재고현황', 1),
  m('stock-ledger', '재고', '재고수불부', 1),
  m('move', '재고', '창고이동', 1),
