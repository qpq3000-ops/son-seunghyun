...
import type { Receivable, Receipt } from '../types';
...
// ───────────────────────── 매출처로부터(회계Ⅰ>현금거래, §4.1) ─────────────────────────
// 단순 입출금 폼(그리드 없음) — POST /api/receipts(kind='수금')를 그대로 재사용한다(설계-R11-입력화면.md §4.1).
type HelpKind = 'account' | 'partner' | null;

export function FromSalePartnerInput() {
  const toast = useToast();
  const [ioDate, setIoDate] = useState(todayISO());
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      // 입금계좌 → method 매핑: 101(현금) 선택 시 '현금', 그 외는 '보통예금'(설계 2.2/4.1)
      ...
      const res = await api.post<Receipt>('/api/receipts', {
        kind: '수금', io_date: ioDate, partner_id: partnerId,
  ...
  return (
    <div className="screen r11b-form">
      <div className="voucher-head">
        <div className="vh-title">매출처로부터</div>
