...
function loadDocDetail(id) {
  const doc = db.prepare(`
    ...
    WHERE d.id = ? AND d.doc_type IN ('sale','purchase')
  `).get(id);
  if (!doc) return null;
  const lines = db.prepare(`
    SELECT dl.id, dl.line_no, dl.item_id, i.code AS item_code, i.name AS item_name, i.unit,
...
vouchers.get('/docs', (c) => {
  ...
  if (!['sale', 'purchase', 'roast'].includes(type)) {
...
vouchers.post('/docs', async (c) => {
  ...
  try {
    ...
    const warnings = stockWarnings(parsed.lines.map(l => ({ itemId: l.item_id, warehouseId })));
...
vouchers.put('/docs/:id', async (c) => {
  const id = validId(c);
  const existing = id
    ? db.prepare(`SELECT * FROM doc WHERE id = ? AND doc_type IN ('sale','purchase')`).get(id)
    : null;
  ...
  const type = existing.doc_type; // 전표 유형·번호는 수정 시 변경하지 않음(불변)
  const ioDate = body.io_date;
  if (!isValidDate(ioDate)) return err(c, 400, '일자를 입력하세요.');
  const partnerId = Number(body.partner_id) || 0;
  if (!partnerId) return err(c, 400, '거래처를 선택하세요.');
  ...
  try {
    db.transaction(() => {
      ...
      db.prepare(`
        UPDATE doc SET partner_id=?, warehouse_id=?, tax_mode=?, project_id=?, memo=?,
          total_qty=?, total_supply=?, total_vat=?, total_amount=?, updated_at=datetime('now','localtime')
        WHERE id=?
      `).run(partnerId, warehouseId, taxMode, projectId, memo,
             round1(totalQty), totalSupply, totalVat, totalSupply + totalVat, id);

      // 원장 재작성: 기존 라인·원장을 지우고 새로 채운다(트랜잭션 안).
...
vouchers.delete('/docs/:id', (c) => {
  const id = validId(c);
  const info = id
    ? db.transaction(() => db.prepare(`DELETE FROM doc WHERE id = ?`).run(id))()
    : { changes: 0 };
